  ...
  "episodes": [
    {
      ...
      const data = await res.json()
      const text = data.content?.[0]?.text || '{}'
      setBible(JSON.parse(text.replace(/```json
?/g,'').replace(/```
?/g,'').trim()))
    } catch (e) { console.error(e) }
    setGenerating(false)
          ...
          <div style={{ display:'flex',minHeight:'100vh',background:'var(--bg)' }}>
            <aside style={{ width:'225px',background:'var(--bg2)',borderRight:'0.5px solid rgba(107,33,168,0.12)',flexShrink:0,height:'100vh',position:'sticky',top:0,overflowY:'auto',display:'flex',flexDirection:'column' }}>
              ...
              <div style={{ padding:'10px',flex:1,overflowY:'auto' }}>
                {NAV.filter(n => n.room !== 'admin' || user?.id === process.env.NEXT_PUBLIC_ADMIN_USER_ID).map(({ room:r, icon, label }) => (
            ...
            <main style={{ flex:1,overflowY:'auto',padding:'28px',background:'radial-gradient(ellipse at 70% 0%,rgba(107,33,168,0.05) 0%,transparent 50%)' }}>
              ...
              {room!=='home' && room!=='birth' && room!=='dna' && room!=='assets' && room!=='memory' && room!=='world' && room!=='brand' && room!=='photoshoot' && room!=='movie' && room!=='blueprint' && room!=='scanner' && room!=='collab' && room!=='admin' && <ComingSoon label={NAV.find(n=>n.room===room)?.label||''} icon={NAV.find(n=>n.room===room)?.icon||'◈'} />}
            </main>
          </div>
        </AccessGate>
      </SignedIn>
