        ...
        <AccessGate>
          <div style={{ display:'flex',minHeight:'100vh',background:'var(--bg)' }}>
            ...
            <main style={{ flex:1,overflowY:'auto',padding:'28px',background:'radial-gradient(ellipse at 70% 0%,rgba(107,33,168,0.05) 0%,transparent 50%)' }}>
              ...
              {room!=='home' && room!=='birth' && room!=='dna' && room!=='assets' && room!=='memory' && room!=='world' && room!=='brand' && room!=='photoshoot' && room!=='movie' && room!=='blueprint' && <ComingSoon label={NAV.find(n=>n.room===room)?.label||''} icon={NAV.find(n=>n.room===room)?.icon||'◈'} />}
            </main>
          </div>
        </AccessGate>
      </SignedIn>
