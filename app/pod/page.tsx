  ...
  async function loadProducts() {
    ...
    try {
      if (provider === 'printful') {
        ...
        setStatus(data.products?.length + ' products loaded')
      } else {
        ...
        const data = await res.json()
        setProducts(data.blueprints?.slice(0, 50) || [])
        setStatus(data.blueprints?.length + ' products loaded')
      }
    } catch (e) { setStatus('Error loading products: ' + (e as Error).message) }
    setLoading(false)
  }
