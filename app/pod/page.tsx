...
function DesignStudio() {
  ...
  const [placement, setPlacement] = useState('default')
  ...
  async function loadVariants(product: { id: number; name: string; icon: string }) {
    setSelectedProduct(product); setVariants([]); setSelectedVariant(null); setPlacement('default')
          ...
          <div>
            ...
            <div className="card hi" style={{ marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' as const }}>
                {(() => {
                  const allOverIds = [189, 200, 303, 198, 589, 507, 477, 737, 558, 279, 465, 350, 924, 262, 83, 350, 630]
                  const opts = selectedProduct && allOverIds.includes(selectedProduct.id)
                    ? ['default', 'left_leg', 'right_leg', 'belt_front']
                    : ['front', 'back', 'sleeve_left', 'sleeve_right']
                  return opts.map(p => <button key={p} onClick={() => setPlacement(p)} style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '10px', cursor: 'pointer', border: `0.5px solid ${placement === p ? 'rgba(255,34,85,0.5)' : 'rgba(255,34,85,0.15)'}`, background: placement === p ? 'rgba(255,34,85,0.1)' : 'transparent', color: placement === p ? 'var(--red)' : 'var(--mu3)', fontFamily: "'DM Sans',sans-serif", textTransform: 'capitalize' as const }}>{p.replace(/_/g,' ')}</button>)
                })()}
              </div>
            </div>
