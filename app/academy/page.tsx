...
// ── ACCESS GATE ───────────────────────────────────────────────
function AcademyAccessGate({ children }: { children: React.ReactNode }) {
  const { user } = useUser()
  ...
  useEffect(() => {
    async function check() {
      ...
      try {
        ...
        if (user.id === adminId) { setHasAccess(true); return }
  ...
  if (hasAccess === null) return (
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: '120px', height: '2px', background: '#1a0030', borderRadius: '1px', overflow: 'hidden', margin: '0 auto 16px' }}>
          <div style={{ height: '100%', background: 'linear-gradient(135deg,#7c3aed,#b06cff,#e8c76a)', backgroundSize: '200% 100%', animation: 'lbar 2s linear infinite' }} />
        </div>
        <div style={{ fontSize: '12px', color: 'rgba(176,108,255,0.4)', fontFamily: "'DM Mono',monospace" }}>Verifying access...</div>
      </div>
    </div>
  )
  ...
  if (!hasAccess) return (
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ maxWidth: '480px', width: '100%', textAlign: 'center' }}>
        <div style={{ background: '#0e0a18', border: '0.5px solid rgba(176,108,255,0.25)', borderRadius: '20px', padding: '40px 32px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: 'linear-gradient(135deg,#7c3aed,#b06cff,#e8c76a)' }} />
          <div style={{ fontSize: '48px', marginBottom: '16px' }}>🎓</div>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '28px', fontWeight: 400, color: '#f8f0ff', marginBottom: '8px' }}>
            Academy Access <span style={{ background: 'linear-gradient(135deg,#7c3aed,#b06cff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Required</span>
          </div>
          <div style={{ fontSize: '13px', color: 'rgba(224,200,255,0.5)', lineHeight: '1.7', marginBottom: '28px' }}>
            Envi Lee Baddie Academy and Kings Academy are exclusive programs. You must enroll through the official website to access these tools.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
            <a href="https://envileebaddieacademy.com" target="_blank" rel="noreferrer"
              style={{ display: 'block', padding: '13px 24px', borderRadius: '10px', background: 'linear-gradient(135deg,#7c3aed,#b06cff)', color: '#fff', fontSize: '13px', fontWeight: 700, textDecoration: 'none' }}>
              Join Baddie Academy →
            </a>
            <a href="https://envileekingsacademy.com" target="_blank" rel="noreferrer"
              style={{ display: 'block', padding: '13px 24px', borderRadius: '10px', background: 'linear-gradient(135deg,#c4973a,#e8c76a)', color: '#000', fontSize: '13px', fontWeight: 700, textDecoration: 'none' }}>
              Join Kings Academy →
            </a>
          </div>
          <div style={{ marginTop: '20px', fontSize: '11px', color: 'rgba(176,108,255,0.3)' }}>
            Already enrolled? Make sure you signed in with the same email you enrolled with.
          </div>
        </div>
      </div>
    </div>
