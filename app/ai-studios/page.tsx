...
function EpisodeBuilder() {
  ...
  function buildExportText(ep: EBEpisode) {
    return [
      `ENVI LEE CREATOR STUDIOS — EPISODE ${ep.number}: "${ep.title.toUpperCase()}"`,
      "=".repeat(60),
      `VIBE: ${ep.vibe} | RUNTIME: ${ep.runtime} | PLATFORMS: ${ep.platforms.join(", ")}`,
      ep.synopsis ? `
SYNOPSIS
${ep.synopsis}` : "",
      "
...
" + "=".repeat(60) + "
REALISM CHECKLIST",
      ...EB_REALISM_RULES.map(r => `${ep.realism[r.id] ? "✓" : "○"} ${r.label}`),
    ].join("
")
  }
        ...
        {scene && (
          <div style={{ marginTop:24 }}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <input style={{ ...ebS.input, fontFamily:"Georgia,serif", fontStyle:"italic", fontSize:15, padding:"12px 16px" }} value={scene.voiceover} onChange={e=>updateScene("voiceover",e.target.value)} placeholder='"She didn't have to say a word. The room already answered."' />
            </div>
...
function SceneVideoGenerator() {
  ...
  const videoModels = [
    { id: 'kling-video', label: 'Kling AI (fal.ai)', desc: 'Best motion, most realistic. Uses FAL_API_KEY', provider: 'fal' },
    { id: 'runway', label: 'Runway Gen-4', desc: 'Cinematic quality, great camera control. Needs RUNWAY_API_KEY', provider: 'runway' },
    { id: 'higgsfield', label: 'Higgsfield OmniHuman', desc: 'Best for human motion + lip sync. Needs Plus plan', provider: 'higgsfield' },
  ]
...
SCENES IN ORDER:
${scenes.map((s, i) => `${String(i+1).padStart(2,'0')}. ${s.title} [${s.duration}s] → Transition: ${s.transition}`).join('
')}
