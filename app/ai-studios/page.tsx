...
function SceneVideoGenerator() {
  ...
  useEffect(() => {
    const stills = JSON.parse(localStorage.getItem(`scene_stills_${user?.id}`) || '{}')
    setSavedStills(stills)
  }, [user?.id])

  const videoModels = [
    { id: 'kling-video', label: 'Kling AI (via fal.ai)', desc: 'Best motion, most realistic. Great for scenes and B-roll.', provider: 'fal' },
    { id: 'higgsfield', label: 'Higgsfield DoP', desc: 'Cinematic quality, director controls. Best for your AI twin content.', provider: 'higgsfield' },
    { id: 'higgsfield-kling', label: 'Higgsfield + Kling 2.1', desc: 'Kling 2.1 Pro through Higgsfield — advanced cinematic animations.', provider: 'higgsfield' },
    { id: 'runway', label: 'Runway Gen-4', desc: 'Great camera control. Needs RUNWAY_API_KEY.', provider: 'runway' },
  ]

  const motionOptions = [
    { id: 'subtle', label: 'Subtle — minimal movement, almost still' },
    { id: 'medium', label: 'Medium — natural gentle motion' },
    { id: 'dynamic', label: 'Dynamic — strong camera movement' },
    { id: 'cinematic', label: 'Cinematic — slow dramatic movement' },
  ]
  ...
  async function generateVideo() {
    ...
    setGenerating(true)
    try {
      const motionPrompt = `${customPrompt || selectedStill.prompt}, ${motion} motion, cinematic video, smooth camera movement, ultra realistic`
      const res = await fetch('/api/generate/video', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageUrl: selectedStill.imageUrl,
          prompt: motionPrompt,
          model: videoModel,
      ...
      const data = await res.json()
      if (data.videoUrl) {
        setVideos(prev => ({ ...prev, [selectedStill.id]: data.videoUrl }))
        const savedVideos = JSON.parse(localStorage.getItem(`scene_videos_${user?.id}`) || '{}')
        savedVideos[selectedStill.id] = { videoUrl: data.videoUrl, sceneTitle: selectedStill.sceneTitle }
        localStorage.setItem(`scene_videos_${user?.id}`, JSON.stringify(savedVideos))
      } else if (data.jobId) {
        setJobId(data.jobId)
        setPolling(true)
        pollForVideo(data.jobId, selectedStill.id)
      }
    } catch (e) { console.error(e) }
    setGenerating(false)
  }

  async function pollForVideo(jId: string, sceneId: string) {
    let attempts = 0
    while (attempts < 30) {
      await new Promise(r => setTimeout(r, 3000))
      try {
        const res = await fetch(`/api/generate/video?jobId=${jId}&model=${videoModel}`)
        const data = await res.json()
        if (data.videoUrl) {
          setVideos(prev => ({ ...prev, [sceneId]: data.videoUrl }))
          const savedVideos = JSON.parse(localStorage.getItem(`scene_videos_${user?.id}`) || '{}')
          savedVideos[sceneId] = { videoUrl: data.videoUrl, sceneTitle: selectedStill?.sceneTitle || '' }
          localStorage.setItem(`scene_videos_${user?.id}`, JSON.stringify(savedVideos))
          setPolling(false)
          return
        }
  ...
  const stillEntries = Object.entries(savedStills)

  return (
    <div className="pg-in">
      <div style={{ marginBottom: '24px' }}>
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '10px', color: 'rgba(6,182,212,0.5)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '4px' }}>Pipeline Step 2</div>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '32px', fontWeight: 400, color: 'var(--w)', marginBottom: '4px' }}>
          Scene Video <span style={{ background: 'var(--ai-grad)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Generator</span>
        </div>
        <div style={{ fontSize: '13px', color: 'var(--mu3)' }}>Animate your scene stills into video clips — choose Kling, Runway, or Higgsfield. Then take clips to Voice Studio and Lip Sync.</div>
      </div>

      {/* Pipeline */}
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap' as const }}>
        {[['🎬 Episode Builder', false], ['→', false], ['🖼️ Scene Stills', false], ['→', false], ['🎥 Scene Videos', true], ['→', false], ['🎙️ Voice + Lip Sync', false], ['→', false], ['🎞️ Episode Editor', false]].map(([label, active], i) => (
          label === '→' ? <span key={i} style={{ color: 'var(--mu2)', fontSize: '16px' }}>→</span> :
          <span key={i} style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontFamily: "'DM Mono',monospace", background: active ? 'var(--ai-grad)' : 'transparent', color: active ? '#000' : 'var(--mu3)', border: active ? 'none' : '0.5px solid rgba(0,207,255,0.15)' }}>{label as string}</span>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px' }}>
        ...
        <div>
          <div className="card hi" style={{ marginBottom: '14px' }}>
            <div className="ftitle">Video Settings</div>
            <F label="Video Generation Model">
              {videoModels.map(m => (
                <div key={m.id} onClick={() => setVideoModel(m.id)}
                  style={{ padding: '10px 12px', borderRadius: '8px', border: `0.5px solid ${videoModel === m.id ? 'rgba(0,207,255,0.4)' : 'rgba(0,207,255,0.1)'}`, background: videoModel === m.id ? 'rgba(0,207,255,0.06)' : 'transparent', cursor: 'pointer', marginBottom: '6px', transition: 'all .15s' }}>
                  <div style={{ fontSize: '12px', fontWeight: 600, color: videoModel === m.id ? 'var(--cyan)' : 'var(--w)' }}>{m.label}</div>
                  <div style={{ fontSize: '10px', color: 'var(--mu3)', marginTop: '2px' }}>{m.desc}</div>
                </div>
              ))}
            </F>
            <F label="Duration">
              <div style={{ display: 'flex', gap: '6px' }}>
                {['3', '5', '10'].map(d => (
                  <button key={d} onClick={() => setDuration(d)} style={{ flex: 1, padding: '8px', borderRadius: '7px', fontSize: '12px', cursor: 'pointer', border: `0.5px solid ${duration === d ? 'rgba(0,207,255,0.4)' : 'rgba(0,207,255,0.1)'}`, background: duration === d ? 'rgba(0,207,255,0.08)' : 'transparent', color: duration === d ? 'var(--cyan)' : 'var(--mu3)', fontFamily: "'DM Sans',sans-serif" }}>{d}s</button>
                ))}
              </div>
            </F>
            <F label="Motion Style">
              <select className="fsel" value={motion} onChange={e => setMotion(e.target.value)}>
                {motionOptions.map(m => <option key={m.id} value={m.id}>{m.label}</option>)}
              </select>
            </F>
            <F label="Custom Motion Prompt (optional)">
              <textarea className="fta" style={{ minHeight: '70px' }} placeholder="e.g. Slow zoom in, camera drifts left, shallow depth of field..." value={customPrompt} onChange={e => setCustomPrompt(e.target.value)} />
            </F>
          </div>

          <button className="ai-btn" onClick={generateVideo} disabled={generating || polling || !selectedStill} style={{ width: '100%', fontSize: '13px', marginBottom: '8px' }}>
            {generating ? '🎥 Submitting job...' : polling ? '⏳ Processing video...' : '🎥 Generate Video Clip'}
          </button>
          {(generating || polling) && <div className="lbar" style={{ marginTop: '6px' }}><div className="lbar-fill" /></div>}
          {polling && <div style={{ fontSize: '11px', color: 'var(--mu3)', textAlign: 'center', marginTop: '6px' }}>Video generation takes 30-90 seconds — polling for result...</div>}
        </div>

        {/* Scene stills to select + output */}
        <div>
          {stillEntries.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '60px 20px', opacity: 0.5 }}>
              <div style={{ fontSize: '48px', marginBottom: '12px', opacity: 0.3 }}>🖼️</div>
              <div style={{ fontSize: '13px', color: 'var(--mu3)' }}>Generate scene stills first in Scene Still Generator</div>
            </div>
          ) : (
            <div>
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '10px', color: 'rgba(6,182,212,0.4)', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '12px' }}>
                Select a Still to Animate ({stillEntries.length} available)
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '20px' }}>
                {stillEntries.map(([id, still]) => (
                  <div key={id} onClick={() => setSelectedStill({ ...still, id })}
                    style={{ borderRadius: '10px', overflow: 'hidden', cursor: 'pointer', border: `2px solid ${selectedStill?.id === id ? 'var(--cyan)' : 'transparent'}`, transition: 'all .2s' }}>
                    <img src={still.imageUrl} alt={still.sceneTitle} style={{ width: '100%', height: '120px', objectFit: 'cover', display: 'block' }} />
                    <div style={{ padding: '8px 10px', background: 'var(--s1)' }}>
                      <div style={{ fontSize: '11px', fontWeight: 600, color: selectedStill?.id === id ? 'var(--cyan)' : 'var(--w)' }}>{still.sceneTitle}</div>
                      {videos[id] && <div style={{ fontSize: '10px', color: '#00ff88', marginTop: '2px' }}>✓ Video ready</div>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Video output */}
              {selectedStill && videos[selectedStill.id] && (
                <div className="card hi" style={{ borderColor: 'rgba(0,207,255,0.25)' }}>
                  <div className="ftitle" style={{ color: 'var(--cyan)' }}>✓ {selectedStill.sceneTitle} — Video Ready</div>
                  <video controls src={videos[selectedStill.id]} style={{ width: '100%', borderRadius: '8px', marginBottom: '12px' }} />
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <a href={videos[selectedStill.id]} download style={{ padding: '8px 18px', borderRadius: '8px', background: 'var(--ai-grad)', color: '#000', fontSize: '12px', fontWeight: 700, textDecoration: 'none' }}>⬇ Download</a>
                    <button onClick={() => { window.location.hash = 'lipsync' }} className="ghost-ai" style={{ fontSize: '12px' }}>Take to Lip Sync →</button>
                  </div>
                </div>
              )}

              {Object.keys(videos).length > 0 && (
                <div style={{ marginTop: '12px', padding: '12px 16px', background: 'rgba(0,207,255,0.05)', border: '0.5px solid rgba(0,207,255,0.2)', borderRadius: '10px', fontSize: '12px', color: 'var(--cyan)' }}>
                  ✦ {Object.keys(videos).length} videos generated — go to <strong>Episode Editor™</strong> to arrange them
                </div>
              )}
            </div>
          )}
