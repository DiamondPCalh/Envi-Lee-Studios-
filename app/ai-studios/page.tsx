...
// ── TYPES ─────────────────────────────────────────────────────
type StudioTool = 'dashboard' | 'content' | 'cineflow' | 'lipsync' | 'show' | 'animation' | 'image' | 'video' | 'network' | 'saved' | 'voice' | 'episodebuilder' | 'scenestills' | 'scenevideos' | 'episodeeditor'
  ...
  const VIDEO_CATEGORIES = [
    {
      ...
      videos: [
        { id: 'r1', title: 'CEO Baddie Day in My Life', creator: 'Nova Star', tools: ['Kling AI', 'ElevenLabs', 'Higgsfield'], prompts: ['Black woman CEO luxury penthouse morning, designer outfit, golden hour, Sony A7R IV', 'Slow dolly forward, Black woman looking out at city skyline'], workflow: 'Step 1: Generated character stills in Image Generator with face locked. Step 2: Animated each scene using Kling AI. Step 3: Added voiceover in ElevenLabs. Step 4: Lip synced in OmniHuman. Step 5: Edited in CapCut.', thumbnail: '', duration: '2:34', category: 'Reality Shows' },
        { id: 'r2', title: 'Luxury Morning Routine', creator: 'Luxe B', tools: ['Higgsfield DoP', 'Sync.so', 'VEED Fabric'], prompts: ['Black woman luxury bathroom, silk robe, warm morning light, bokeh background'], workflow: 'Step 1: Episode Builder for scene planning. Step 2: Generated stills per scene. Step 3: Animated with Higgsfield DoP. Step 4: Lip synced with VEED Fabric 1.0.', thumbnail: '', duration: '1:45', category: 'Reality Shows' },
      ]
    },
    {
      id: 'talkshow', label: 'Talk Shows', icon: '🎤',
      videos: [
        { id: 't1', title: 'The Baddie Couch EP 1', creator: 'Diamond Lee', tools: ['VEED Fabric 1.0', 'ElevenLabs', 'Runway Gen-4'], prompts: ['Two Black women on luxury talk show set, professional lighting, editorial look'], workflow: 'Step 1: Built characters in Character Builder. Step 2: Generated talk show set with Image Generator. Step 3: Used 4-character lip sync for dialogue. Step 4: Animated with Runway Gen-4.', thumbnail: '', duration: '5:12', category: 'Talk Shows' },
      ]
    },
    {
      id: 'episodes', label: 'Episodes', icon: '🎬',
      videos: [
        { id: 'e1', title: 'The Arrival — Episode 01', creator: 'Nova Star', tools: ['Episode Builder', 'Kling AI', 'OmniHuman'], prompts: ['Cinematic arrival scene, Black woman in luxury hotel lobby, slow motion, film grain'], workflow: 'Step 1: Wrote episode in Episode Builder. Step 2: Generated scene stills. Step 3: Animated with Kling AI. Step 4: Episode Editor for final cut.', thumbnail: '', duration: '3:20', category: 'Episodes' },
      ]
    },
    {
      id: 'tutorials', label: 'Tutorials', icon: '📚',
      videos: [
        { id: 'tu1', title: 'How I Generated My AI Twin', creator: 'Creator101', tools: ['Realism Studio', 'Image Generator', 'Flux Kontext'], prompts: ['Ultra realistic Black woman, natural skin texture, no AI look, Sony A7R IV'], workflow: 'Step 1: Used Realism Studio for base character. Step 2: Locked face with Flux Kontext. Step 3: Generated multiple outfits and scenes.', thumbnail: '', duration: '4:01', category: 'Tutorials' },
      ]
    },
    {
    ...
    },
    {
      ...
      videos: [
        { id: 'sk1', title: 'Baddie vs Karen — Skit', creator: 'Nova Star', tools: ['Kling AI', 'VEED Fabric', 'ElevenLabs'], prompts: ['Comedy skit, two characters, bright lighting, clear facial expressions'], workflow: 'Step 1: Wrote script. Step 2: Built characters. Step 3: Generated scenes. Step 4: Added dialogue with 2-character lip sync.', thumbnail: '', duration: '1:12', category: 'Skits' },
      ]
    },
  ]
...
function ImageGenerator() {
  ...
  const [imgMode, setImgMode] = useState<'single' | 'elements' | 'multichar'>('single')
  const [imgElements, setImgElements] = useState<{ id: string; url: string; label: string }[]>([])
  const [imgChars, setImgChars] = useState<{ id: string; url: string; name: string }[]>([])
  const imgElLabels = ['Background', 'Character', 'Object', 'Style Ref']
  const imgElRefs = [useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null)]
  const imgCharRefs = [useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null)]
  ...
  async function generate() {
    ...
    try {
      ...
      if (imgMode === 'multichar' && imgChars.filter(Boolean).length > 0) {
        const names = imgChars.filter(Boolean).map(c => c?.name || 'Character').join(', ')
        enhancedPrompt += '. All characters in same scene: ' + names + '. Each person clearly visible, distinct faces, natural poses together.'
      }
      if (imgMode === 'elements' && imgElements.length > 0) {
        enhancedPrompt += '. Scene includes: ' + imgElements.map(e => e.label).join(', ') + '. Compose all elements naturally in one cohesive scene.'
      }

      const castPhotos = imgMode === 'multichar' ? imgChars.filter(Boolean).map(c => c!.url) : undefined
      const elementImages = imgMode === 'elements' ? imgElements.map(e => e.url) : undefined
      const activeFacePhoto = imgMode === 'single' ? facePhoto : null

      // For multichar we generate once (Gemini handles multiple in one call)
      // For single we can generate multiple
      const count = imgMode === 'multichar' ? 1 : numImages
      ...
      const promises = Array(count).fill(null).map(() =>
        fetch('/api/generate/image', {
          ...
          model: d.model || '',
          faceLocked: d.faceLocked || false,
        }))
      )
      const results = await Promise.all(promises)
      const urls = results.map(r => r.imageUrl).filter(Boolean) as string[]
      const lastResult = results[results.length - 1]
      setImageUrls(urls)
      if (lastResult.faceLocked) setFaceLocked(true)
      // Show which model was used
      if (lastResult.model) console.log('Generated with:', lastResult.model)
    ...
    <div className="pg-in" style={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' as const }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '20px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
          ...
          <div className="card hi">
            <div className="ftitle">🎨 Generation Mode</div>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '5px' }}>
              {([
                { id: 'single', icon: '🖼️', label: 'Single / Face Lock', desc: 'One image with optional face lock' },
                { id: 'elements', icon: '✦', label: 'Multi Elements', desc: 'Layer background + character + objects' },
                { id: 'multichar', icon: '👥', label: 'Multi Character', desc: 'Up to 4 faces in one image' },
              ] as const).map(m => (
                <div key={m.id} onClick={() => setImgMode(m.id)}
                  style={{ padding: '8px 10px', borderRadius: '8px', cursor: 'pointer', border: `0.5px solid ${imgMode === m.id ? 'rgba(0,212,255,0.5)' : 'rgba(0,212,255,0.1)'}`, background: imgMode === m.id ? 'rgba(0,212,255,0.08)' : 'var(--bg3)', display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <span>{m.icon}</span>
                  <div>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: imgMode === m.id ? 'var(--cyan)' : 'var(--w)' }}>{m.label}</div>
                    <div style={{ fontSize: '10px', color: 'var(--mu3)' }}>{m.desc}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* SINGLE MODE — Face reference */}
          {imgMode === 'single' && (
            <div className="card hi">
              <div className="ftitle">🔒 Face Reference (Optional)</div>
              <DragDrop label="Drag your AI twin photo here" sub="Locks face — PuLID active" currentImage={facePhoto}
                onImage={url => { setFacePhoto(url); setFaceLocked(true) }}
                onClear={() => { setFacePhoto(null); setFaceLocked(false) }} height={100} />
              {facePhoto && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '8px', padding: '6px 10px', background: 'rgba(0,212,255,0.08)', borderRadius: '6px', border: '0.5px solid rgba(0,212,255,0.2)' }}>
                  ...
                  <span style={{ fontSize: '11px', color: 'var(--cyan)', fontFamily: "'DM Mono',monospace" }}>Face locked — PuLID (ByteDance)</span>
                </div>
              )}
            </div>
          )}
          ...
          {imgMode === 'elements' && (
            <div className="card hi">
              <div className="ftitle">✦ Elements — Layer Your Scene</div>
              <div style={{ fontSize: '10px', color: 'var(--mu3)', marginBottom: '10px', lineHeight: '1.5' }}>Upload up to 4 reference images — background, character, object, style ref.</div>
              {imgElLabels.map((label, i) => {
                const el = imgElements.find(e => e.label === label)
                return (
                  <div key={label} style={{ marginBottom: '8px' }}>
                    <div style={{ fontSize: '9px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginBottom: '4px', textTransform: 'uppercase' as const }}>{label}</div>
                    <input ref={imgElRefs[i]} type="file" accept="image/*" style={{ display: 'none' }} onChange={async e => {
                      const f = e.target.files?.[0]; if (!f) return
                      const url = await new Promise<string>(res => { const r = new FileReader(); r.onload = ev => res(ev.target?.result as string); r.readAsDataURL(f) })
                      setImgElements(prev => el ? prev.map(ev => ev.label === label ? { ...ev, url } : ev) : [...prev, { id: Date.now().toString(), url, label }])
                    }} />
                    <div onClick={() => imgElRefs[i].current?.click()}
                      style={{ border: `1px dashed ${el ? 'rgba(0,212,255,0.4)' : 'rgba(0,212,255,0.15)'}`, borderRadius: '6px', height: '65px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--bg3)', overflow: 'hidden', position: 'relative' as const }}>
                      {el
                        ? <><img src={el.url} alt={label} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><button onClick={ev => { ev.stopPropagation(); setImgElements(prev => prev.filter(e => e.label !== label)) }} style={{ position: 'absolute' as const, top: '3px', right: '3px', background: 'rgba(0,0,0,0.7)', border: 'none', color: '#fff', borderRadius: '50%', width: '16px', height: '16px', cursor: 'pointer', fontSize: '9px' }}>✕</button></>
                        : <div style={{ textAlign: 'center' }}><div style={{ fontSize: '16px', opacity: 0.2 }}>+</div><div style={{ fontSize: '9px', color: 'var(--mu3)' }}>Add {label}</div></div>
                      }
                    </div>
                  </div>
                )
              })}
              <div style={{ fontSize: '10px', color: 'rgba(0,212,255,0.5)', fontFamily: "'DM Mono',monospace" }}>{imgElements.length}/4 elements added</div>
            </div>
          )}
          ...
          {imgMode === 'multichar' && (
            <div className="card hi">
              <div className="ftitle">👥 Multi Character — Up to 4 Faces</div>
              <div style={{ fontSize: '10px', color: 'var(--mu3)', marginBottom: '10px', lineHeight: '1.5' }}>Upload character faces. All appear together in one image. First face = primary identity lock.</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                {[0, 1, 2, 3].map(i => {
                  const char = imgChars[i]
                  return (
                    <div key={i}>
                      <div style={{ fontSize: '9px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginBottom: '4px' }}>CHAR {i + 1}{i === 0 ? ' ★' : ''}</div>
                      <input ref={imgCharRefs[i]} type="file" accept="image/*" style={{ display: 'none' }} onChange={async e => {
                        const f = e.target.files?.[0]; if (!f) return
                        const url = await new Promise<string>(res => { const r = new FileReader(); r.onload = ev => res(ev.target?.result as string); r.readAsDataURL(f) })
                        setImgChars(prev => { const u = [...prev]; u[i] = { id: i.toString(), url, name: 'Character ' + (i + 1) }; return u })
                      }} />
                      <div onClick={() => imgCharRefs[i].current?.click()}
                        style={{ border: `1px dashed ${char ? 'rgba(0,212,255,0.4)' : 'rgba(0,212,255,0.15)'}`, borderRadius: '6px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--bg3)', overflow: 'hidden', position: 'relative' as const }}>
                        {char
                          ? <><img src={char.url} alt={'c'+i} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><button onClick={ev => { ev.stopPropagation(); setImgChars(prev => prev.filter((_, idx) => idx !== i)) }} style={{ position: 'absolute' as const, top: '3px', right: '3px', background: 'rgba(0,0,0,0.7)', border: 'none', color: '#fff', borderRadius: '50%', width: '16px', height: '16px', cursor: 'pointer', fontSize: '9px' }}>✕</button></>
                          : <div style={{ textAlign: 'center' }}><div style={{ fontSize: '20px', opacity: 0.2 }}>👤</div><div style={{ fontSize: '9px', color: 'var(--mu3)' }}>Face {i+1}</div></div>
                        }
                      </div>
                      {char && <input className="finp" placeholder="Name..." value={char.name} onChange={e => setImgChars(prev => prev.map((c, idx) => idx === i ? { ...c, name: e.target.value } : c))} style={{ fontSize: '10px', padding: '4px 8px', marginTop: '4px' }} />}
                    </div>
                  )
                })}
              </div>
              <div style={{ fontSize: '10px', color: 'rgba(0,212,255,0.5)', marginTop: '8px', fontFamily: "'DM Mono',monospace" }}>{imgChars.length}/4 characters</div>
            </div>
          )}
            ...
            <div className="card" style={{ height: '100%', minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: '14px', color: 'var(--cyan)', marginBottom: '8px' }}>
                  {facePhoto ? '🔒 Generating with PuLID — same face every time...' : '✦ Generating ultra realistic image...'}
                </div>
...
// Blocks non-students and shows $47/$65 paywall
    ...
    <div style={{ minHeight:'100vh', background:'#000', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
      <div style={{ maxWidth:'520px', width:'100%' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'28px' }}>
          ...
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'32px', fontWeight:400, color:'#fff', marginBottom:'6px' }}>
            {appName}
          </div>
          <div style={{ fontSize:'13px', color:'rgba(255,255,255,0.4)' }}>
            Choose your access level to unlock this app
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'16px' }}>
          ...
          <div style={{ background:'#0d0020', border:'1px solid rgba(168,85,247,0.3)', borderRadius:'16px', padding:'24px', position:'relative' as const, overflow:'hidden' }}>
            ...
            <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.5)', lineHeight:'1.7', marginBottom:'16px' }}>
              Full access to all 12 apps in the Envi Lee Creator Empire
            </div>
            ...
            <a href="/api/stripe/checkout?plan=member" style={{ display:'block', marginTop:'16px', padding:'11px', borderRadius:'9px', background:'linear-gradient(135deg,#6B21A8,#A855F7)', color:'#fff', fontSize:'12px', fontWeight:700, textDecoration:'none', textAlign:'center' as const }}>
              Get Member Access →
            </a>
          </div>
          ...
          <div style={{ background:'#0d0020', border:'1px solid rgba(212,168,67,0.4)', borderRadius:'16px', padding:'24px', position:'relative' as const, overflow:'hidden' }}>
            ...
            <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.5)', lineHeight:'1.7', marginBottom:'16px' }}>
              Everything in Member plus exclusive VIP perks
            </div>
            ...
            <a href="/api/stripe/checkout?plan=vip" style={{ display:'block', marginTop:'16px', padding:'11px', borderRadius:'9px', background:'linear-gradient(135deg,#D4A843,#F5E0A0)', color:'#000', fontSize:'12px', fontWeight:700, textDecoration:'none', textAlign:'center' as const }}>
              Get VIP Access →
            </a>
          </div>
        </div>
        ...
        <div style={{ textAlign:'center', fontSize:'11px', color:'rgba(255,255,255,0.2)' }}>
          Envi Lee Creator Studios™ · All prices in USD · Cancel anytime
        </div>
      </div>
    </div>
  )
...
function VoiceStudio() {
  ...
  const [audioUrl, setAudioUrl] = useState('')
  const [userInfo, setUserInfo] = useState<{charactersUsed: number; charactersLimit: number; tier: string} | null>(null)
  ...
  async function loadVoices() {
    ...
    try {
      const res = await fetch('/api/generate/voice?action=voices')
      const data = await res.json()
      if (data.voices) {
  ...
  async function loadUserInfo() {
    try {
      const res = await fetch('/api/generate/voice?action=user_info')
      const data = await res.json()
  ...
  async function generateVoice() {
    ...
    setGenerating(true); setAudioUrl('')
    try {
      const res = await fetch('/api/generate/voice', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'tts', text: script, voiceId: selectedVoice, stability, similarity }),
      })
      const data = await res.json()
      if (data.audioUrl) setAudioUrl(data.audioUrl)
      else alert(data.error || 'Failed to generate voice')
    } catch (e) { console.error(e) }
    setGenerating(false)
  }
  ...
  return (
    <div className="pg-in">
      <div style={{ marginBottom: '24px' }}>
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '10px', color: 'rgba(168,85,247,0.5)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '4px' }}>ElevenLabs</div>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '32px', fontWeight: 400, color: 'var(--w)', marginBottom: '4px' }}>
          Voice <span style={{ background: 'var(--ai-grad)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Studio™</span>
        </div>
        <div style={{ fontSize: '13px', color: 'var(--mu3)' }}>Generate voice audio from any script — then feed it into Lip Sync Studio to make your AI twin speak in your voice.</div>
      </div>

      {/* User info */}
      {userInfo && (
        <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' as const }}>
          <span className="tag" style={{ background: 'rgba(168,85,247,0.1)', color: '#A855F7', border: '0.5px solid rgba(168,85,247,0.3)' }}>
            Plan: {userInfo.tier}
          </span>
          <span className="tag" style={{ background: 'rgba(0,207,255,0.08)', color: 'var(--cyan)', border: '0.5px solid rgba(0,207,255,0.2)' }}>
            {userInfo.charactersUsed.toLocaleString()} / {userInfo.charactersLimit.toLocaleString()} characters used
          </span>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div>
          <div className="card hi" style={{ marginBottom: '14px' }}>
            <div className="ftitle">🎙️ Voice Setup</div>
            <F label="Select Voice">
              {loadingVoices ? (
                <div style={{ padding: '9px 12px', background: 'var(--bg3)', borderRadius: '7px', fontSize: '12px', color: 'var(--mu3)' }}>Loading voices...</div>
              ) : (
                <select className="fsel" value={selectedVoice} onChange={e => setSelectedVoice(e.target.value)}>
                  {voices.map(v => (
                    <option key={v.voice_id} value={v.voice_id}>{v.name} {v.category === 'cloned' ? '(Your Clone)' : ''}</option>
                  ))}
                </select>
              )}
            </F>
            <F label="Stability (0 = expressive, 1 = consistent)">
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <input type="range" min="0" max="1" step="0.1" value={stability} onChange={e => setStability(parseFloat(e.target.value))} style={{ flex: 1 }} />
                <span style={{ fontSize: '12px', color: 'var(--violet2)', fontFamily: "'DM Mono',monospace", width: '30px' }}>{stability}</span>
              </div>
            </F>
            <F label="Similarity (higher = more like original voice)">
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <input type="range" min="0" max="1" step="0.1" value={similarity} onChange={e => setSimilarity(parseFloat(e.target.value))} style={{ flex: 1 }} />
                <span style={{ fontSize: '12px', color: 'var(--violet2)', fontFamily: "'DM Mono',monospace", width: '30px' }}>{similarity}</span>
              </div>
            </F>
          </div>

          <div className="card hi" style={{ marginBottom: '14px' }}>
            <div className="ftitle">📝 Your Script</div>
            <textarea className="fta" style={{ minHeight: '160px', marginBottom: '10px' }}
              placeholder="Type your script here... This is what your AI twin will say in your voice."
              value={script} onChange={e => setScript(e.target.value)} />
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--mu3)', marginBottom: '10px', fontFamily: "'DM Mono',monospace" }}>
              <span>{script.length} characters</span>
              <span>~${(script.length * 0.00003).toFixed(4)} estimated cost</span>
            </div>
            <button className="ai-btn" onClick={generateVoice} disabled={generating || !script.trim() || !selectedVoice} style={{ width: '100%', fontSize: '13px' }}>
              {generating ? '🎙️ Generating voice…' : '🎙️ Generate Voice Audio'}
            </button>
        ...
        <div>
          {audioUrl ? (
            <div>
              <div className="card hi" style={{ marginBottom: '14px' }}>
                <div className="ftitle">✓ Voice Generated</div>
                <audio controls src={audioUrl} style={{ width: '100%', marginBottom: '14px' }} />
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                  <a href={audioUrl} download="voice_audio.mp3" className="ai-btn" style={{ textDecoration: 'none', fontSize: '12px', padding: '8px 16px' }}>
                    ⬇ Download MP3
                  </a>
                  <button className="ghost-ai" onClick={generateVoice} disabled={generating} style={{ fontSize: '12px' }}>
                    ↺ Regenerate
                  </button>
                </div>
              </div>
              <div className="card" style={{ borderColor: 'rgba(168,85,247,0.2)', background: 'rgba(168,85,247,0.05)' }}>
                <div className="ftitle">Next Step — Lip Sync</div>
                <div style={{ fontSize: '12px', color: 'var(--mu3)', lineHeight: '1.7', marginBottom: '12px' }}>
                  Download this audio file then go to <strong style={{ color: '#A855F7' }}>Lip Sync Studio</strong> — upload your AI twin video + this audio file to make her speak in your voice.
                </div>
                <div style={{ fontSize: '11px', color: 'var(--mu3)', padding: '10px', background: 'var(--bg3)', borderRadius: '8px', lineHeight: '1.7' }}>
                  ✦ Download audio → Go to Lip Sync Studio → Upload video + this audio → Generate
                </div>
              </div>
            </div>
          ) : (
            <div>
              <div className="card" style={{ textAlign: 'center', padding: '60px 20px', opacity: 0.5, marginBottom: '14px' }}>
                <div style={{ fontSize: '48px', marginBottom: '12px', opacity: 0.3 }}>🎙️</div>
                <div style={{ fontSize: '13px', color: 'var(--mu3)' }}>Your generated audio appears here</div>
                <div style={{ fontSize: '11px', color: 'var(--mu2)', marginTop: '6px' }}>Write a script and click Generate</div>
              </div>
              <div className="card" style={{ borderColor: 'rgba(168,85,247,0.15)' }}>
                <div className="ftitle">💡 Pro Tips</div>
                {[
                  'Clone your own voice in ElevenLabs — upload 1-3 minute audio samples',
                  'Lower stability = more expressive and emotional',
                  'Higher similarity = sounds more exactly like your voice clone',
                  'Keep scripts under 2500 characters for best quality',
                  'Download the MP3 then use it in Lip Sync Studio',
                ].map(tip => (
                  <div key={tip} style={{ display: 'flex', gap: '7px', fontSize: '12px', color: 'var(--mu3)', marginBottom: '6px' }}>
...
// ── EPISODE BUILDER™ (Full Version) ──────────────────────────
const EB_VIBES = ["Cinematic", "Editorial", "Luxury Reality", "Raw & Real", "Mysterious", "Hype Energy"]
const EB_PLATFORMS = ["TikTok", "Instagram Reels", "YouTube", "Skool (Members)", "Pinterest", "X / Twitter"]
const EB_SCENE_TAGS = ["Arrival", "Power Moment", "AI Lab / Classroom", "Hallway / Movement", "Reaction", "Transition", "Close / Title Card", "Behind the Scenes", "Voiceover Only"]
const EB_SHOT_ANGLES = ["Low angle (street level)", "Eye level", "Tracking / Following", "Wide establishing", "Close-up / Detail", "Over the shoulder", "POV / First person", "Doorway / Frame shot"]
const EB_TIME_OF_DAY = ["Dawn / Early morning", "Morning golden hour", "Midday sharp light", "Late afternoon", "Evening / Warm lamp", "Night / Moody dark"]
const EB_REALISM_RULES = [
  { id: "grain", label: "Slight film grain on every image" },
  { id: "framing", label: "Off-center / imperfect framing" },
  { id: "blur", label: "Motion blur on background figures" },
  { id: "clutter", label: "Environmental clutter in scene" },
  { id: "anchor", label: "Anchor reference image used for face consistency" },
  { id: "lighting", label: "Lighting inconsistency between scenes is okay" },
]
const EB_PLATFORM_GUIDES: Record<string, string> = {
  "TikTok": "60-90 sec cut. Hook in first 2 sec. Captions only, no voiceover narration.",
  "Instagram Reels": "Same as TikTok cut. Use the power shot as the cover frame.",
  "YouTube": "Full episode 3-5 min. Add 30-sec intro + CTA at end to enroll.",
  "Skool (Members)": "Full director cut + prompts used. This IS the curriculum.",
  "Pinterest": "Still frames as editorial pins. Link to sales page.",
  "X / Twitter": "One scene only — the strongest shot. One caption. No context needed.",
}

interface EBScene {
  id: string; title: string; tag: string; description: string
  shotAngle: string; location: string; cameraFeel: string
  wardrobe: string; lighting: string; keyVisual: string
  aiPrompt: string; voiceover: string; platformNotes: string
}
interface EBEpisode {
  id: string; number: string; title: string; vibe: string
  platforms: string[]; runtime: string; synopsis: string
  scenes: EBScene[]; realism: Record<string, boolean>; createdAt: string
}

function ebNewScene(idx: number): EBScene {
  return { id: Date.now().toString() + idx, title: "", tag: "Arrival", description: "", shotAngle: "", location: "", cameraFeel: "", wardrobe: "", lighting: "", keyVisual: "", aiPrompt: "", voiceover: "", platformNotes: "" }
}
function ebNewEpisode(): EBEpisode {
  return { id: Date.now().toString(), number: "", title: "", vibe: "Cinematic", platforms: [], runtime: "", synopsis: "", scenes: [ebNewScene(0)], realism: EB_REALISM_RULES.reduce((a, r) => ({ ...a, [r.id]: false }), {}), createdAt: new Date().toLocaleDateString() }
}
...
function SceneVideoGenerator() {
  ...
  // Elements state
  const [mode, setMode] = useState<'single' | 'elements' | 'multichar'>('single')
  const [elements, setElements] = useState<{ id: string; url: string; label: string; publicUrl?: string }[]>([])
  const [characters, setCharacters] = useState<{ id: string; url: string; name: string; publicUrl?: string }[]>([])
  const elementLabels = ['Background', 'Character', 'Object', 'Overlay']
  const elementRefs = [useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null)]
  const charRefs = [useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null)]
    ...
    try {
      ...
      async function uploadImage(base64: string) {
        const r = await fetch('/api/upload/image', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ imageBase64: base64, folder: 'envi-lee-video' }),
        })
        const d = await r.json()
        return d.imageUrl || null
      }

      const mainImageUrl = uploadedImage || startFrame ? await uploadImage(uploadedImage || startFrame || '') : null
      if (!mainImageUrl) { setStatus('Upload failed'); setGenerating(false); return }

      // Upload elements if in elements mode
      let elementUrls: string[] = []
      if (mode === 'elements' && elements.length > 0) {
        elementUrls = (await Promise.all(elements.map(el => uploadImage(el.url)))).filter(Boolean) as string[]
      }

      // Upload character faces if in multichar mode
      let charUrls: string[] = []
      if (mode === 'multichar' && characters.length > 0) {
        charUrls = (await Promise.all(characters.map(c => uploadImage(c.url)))).filter(Boolean) as string[]
      }

      // Build enhanced prompt
      let motionPrompt = (prompt || 'Cinematic scene') + ', ' + motion + ' motion, cinematic video, smooth camera movement, ultra realistic'
      if (mode === 'elements' && elements.length > 0) {
        motionPrompt += '. Scene contains: ' + elements.map(el => el.label).join(', ') + '. Compose all elements naturally in frame.'
      }
      if (mode === 'multichar' && characters.length > 0) {
        motionPrompt += '. Multiple characters in scene: ' + characters.map(c => c.name || 'Character').join(', ') + '. Keep each character distinct and clearly visible.'
      }
      ...
      const res = await fetch('/api/generate/video', {
        ...
        body: JSON.stringify({
          ...
          elementUrls: elementUrls.length > 0 ? elementUrls : undefined,
          charUrls: charUrls.length > 0 ? charUrls : undefined,
          prompt: motionPrompt,
          model: videoModel,
  ...
  async function pollVideo(jobId: string) {
    let attempts = 0
    const maxAttempts = 60 // 4 mins total
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (data.status === 'failed' || data.error) {
          setStatus('Generation failed: ' + (data.error || 'Try again'))
          setPolling(false)
          return
        }
        if (data.videoUrl || (data.status === 'completed' && data.videoUrl)) {
          setGeneratedVideo(data.videoUrl)
          saveVideoToLibrary(data.videoUrl, prompt)
          setPolling(false)
          setStatus('Video ready! ✓')
        ...
        setStatus('Processing... ' + (attempts + 1) + '/' + maxAttempts + ' · ' + (data.status || 'queued'))
      } catch (e) { console.error(e) }
      attempts++
    }
    setPolling(false)
    setStatus('Timed out — video may still be generating. Check again shortly.')
  }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 260px', gap: '14px', flex: 1, minHeight: 0 }}>
          ...
          <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
            ...
            <div className="card hi">
              ...
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '5px' }}>
                {[
                  { id: 'single', icon: '🖼️', label: 'Single Image', desc: 'One image → video' },
                  { id: 'elements', icon: '✦', label: 'Multi Elements', desc: 'Layer background + characters + objects' },
                  { id: 'multichar', icon: '👥', label: 'Multi Character', desc: 'Multiple faces in one frame' },
                ].map(m => (
                  <div key={m.id} onClick={() => setMode(m.id as 'single' | 'elements' | 'multichar')}
                    style={{ padding: '9px 12px', borderRadius: '8px', cursor: 'pointer', border: `0.5px solid ${mode === m.id ? 'rgba(0,212,255,0.5)' : 'rgba(0,212,255,0.1)'}`, background: mode === m.id ? 'rgba(0,212,255,0.08)' : 'var(--bg3)', display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <span style={{ fontSize: '16px' }}>{m.icon}</span>
                    <div>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: mode === m.id ? 'var(--cyan)' : 'var(--w)' }}>{m.label}</div>
                      <div style={{ fontSize: '10px', color: 'var(--mu3)' }}>{m.desc}</div>
                    </div>
                  </div>
                ))}
            ...
            {mode === 'single' && (
              <div className="card hi">
                ...
                <div onClick={() => imageRef.current?.click()} style={{ border: '1.5px dashed rgba(0,212,255,0.25)', borderRadius: '8px', minHeight: '130px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--bg3)', overflow: 'hidden' }}>
                  {uploadedImage
                    ? <img src={uploadedImage} alt="uploaded" style={{ width: '100%', display: 'block' }} />
                    : <div style={{ textAlign: 'center', padding: '20px' }}><div style={{ fontSize: '24px', opacity: 0.2 }}>🖼️</div><div style={{ fontSize: '11px', color: 'var(--mu3)', marginTop: '6px' }}>Click to upload</div></div>
                  }
                </div>
                {uploadedImage && <button onClick={() => setUploadedImage(null)} style={{ fontSize: '10px', color: '#ff6b6b', background: 'none', border: 'none', cursor: 'pointer', marginTop: '6px' }}>✕ Remove</button>}
              </div>
            )}
            ...
            {mode === 'elements' && (
              <div className="card hi">
                <div className="ftitle">✦ Elements — Layer Your Scene</div>
                <div style={{ fontSize: '10px', color: 'var(--mu3)', marginBottom: '10px', lineHeight: '1.5' }}>Upload up to 4 elements — background, characters, objects. They get composed into one scene.</div>
                {elementLabels.map((label, i) => {
                  const el = elements.find(e => e.label === label)
                  return (
                    <div key={label} style={{ marginBottom: '8px' }}>
                      <div style={{ fontSize: '9px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginBottom: '4px', textTransform: 'uppercase' as const }}>{label}</div>
                      <input ref={elementRefs[i]} type="file" accept="image/*" style={{ display: 'none' }} onChange={async e => {
                        const f = e.target.files?.[0]
                        if (!f) return
                        const url = await new Promise<string>((res) => { const r = new FileReader(); r.onload = ev => res(ev.target?.result as string); r.readAsDataURL(f) })
                        setElements(prev => el ? prev.map(ev => ev.label === label ? { ...ev, url } : ev) : [...prev, { id: Date.now().toString(), url, label }])
                      }} />
                      <div onClick={() => elementRefs[i].current?.click()}
                        style={{ border: `1px dashed ${el ? 'rgba(0,212,255,0.4)' : 'rgba(0,212,255,0.15)'}`, borderRadius: '6px', height: '70px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--bg3)', overflow: 'hidden', position: 'relative' as const }}>
                        {el
                          ? <><img src={el.url} alt={label} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><button onClick={ev => { ev.stopPropagation(); setElements(prev => prev.filter(e => e.label !== label)) }} style={{ position: 'absolute' as const, top: '4px', right: '4px', background: 'rgba(0,0,0,0.7)', border: 'none', color: '#fff', borderRadius: '50%', width: '18px', height: '18px', cursor: 'pointer', fontSize: '10px' }}>✕</button></>
                          : <div style={{ textAlign: 'center' }}><div style={{ fontSize: '18px', opacity: 0.2 }}>+</div><div style={{ fontSize: '9px', color: 'var(--mu3)' }}>Add {label}</div></div>
                        }
                      </div>
                    </div>
                  )
                })}
                <div style={{ fontSize: '10px', color: 'rgba(0,212,255,0.5)', marginTop: '6px', fontFamily: "'DM Mono',monospace" }}>{elements.length}/4 elements added</div>
              </div>
            )}
            ...
            {mode === 'multichar' && (
              <div className="card hi">
                <div className="ftitle">👥 Characters — Multiple Faces in One Frame</div>
                <div style={{ fontSize: '10px', color: 'var(--mu3)', marginBottom: '10px', lineHeight: '1.5' }}>Upload up to 4 character faces. All characters appear together in one scene.</div>

                {/* Base scene image */}
                <div style={{ marginBottom: '10px' }}>
                  <div style={{ fontSize: '9px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginBottom: '4px' }}>BASE SCENE IMAGE</div>
                  <input ref={imageRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => handleImageUpload(e, 'main')} />
                  <div onClick={() => imageRef.current?.click()} style={{ border: '1px dashed rgba(0,212,255,0.2)', borderRadius: '6px', height: '70px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--bg3)', overflow: 'hidden' }}>
                    {uploadedImage ? <img src={uploadedImage} alt="scene" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ textAlign: 'center' }}><div style={{ fontSize: '18px', opacity: 0.2 }}>🌆</div><div style={{ fontSize: '9px', color: 'var(--mu3)' }}>Scene background</div></div>}
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  {[0, 1, 2, 3].map(i => {
                    ...
                    return (
                      <div key={i}>
                        <div style={{ fontSize: '9px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginBottom: '4px' }}>CHARACTER {i + 1}</div>
                        <input ref={charRefs[i]} type="file" accept="image/*" style={{ display: 'none' }} onChange={async e => {
                          const f = e.target.files?.[0]
                          if (!f) return
                          const url = await new Promise<string>((res) => { const r = new FileReader(); r.onload = ev => res(ev.target?.result as string); r.readAsDataURL(f) })
                          setCharacters(prev => {
                            const updated = [...prev]
                            updated[i] = { id: i.toString(), url, name: 'Character ' + (i + 1) }
                            return updated
                          })
                        }} />
                        <div onClick={() => charRefs[i].current?.click()}
                          style={{ border: `1px dashed ${char ? 'rgba(0,212,255,0.4)' : 'rgba(0,212,255,0.15)'}`, borderRadius: '6px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--bg3)', overflow: 'hidden', position: 'relative' as const }}>
                          {char
                            ? <><img src={char.url} alt={'char' + i} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><button onClick={ev => { ev.stopPropagation(); setCharacters(prev => prev.filter((_, idx) => idx !== i)) }} style={{ position: 'absolute' as const, top: '3px', right: '3px', background: 'rgba(0,0,0,0.7)', border: 'none', color: '#fff', borderRadius: '50%', width: '16px', height: '16px', cursor: 'pointer', fontSize: '9px' }}>✕</button></>
                            : <div style={{ textAlign: 'center' }}><div style={{ fontSize: '20px', opacity: 0.2 }}>👤</div><div style={{ fontSize: '9px', color: 'var(--mu3)', marginTop: '2px' }}>Face {i + 1}</div></div>
                          }
                        </div>
                        {char && (
                          <input className="finp" placeholder="Name..." value={char.name} onChange={e => setCharacters(prev => prev.map((c, idx) => idx === i ? { ...c, name: e.target.value } : c))} style={{ fontSize: '10px', padding: '4px 8px', marginTop: '4px' }} />
                        )}
                      </div>
                    )
                  })}
                </div>
                <div style={{ fontSize: '10px', color: 'rgba(0,212,255,0.5)', marginTop: '8px', fontFamily: "'DM Mono',monospace" }}>{characters.length}/4 characters added</div>
              </div>
            )}
...
function EpisodeEditor() {
  const { user } = useUser()
  const [scenes, setScenes] = useState<Array<{id: string; title: string; videoUrl: string; duration: number; transition: string}>>([])
  const [activeScene, setActiveScene] = useState<string | null>(null)

  useEffect(() => {
    const savedVideos = JSON.parse(localStorage.getItem(`scene_videos_${user?.id}`) || '{}')
    const sceneList = Object.entries(savedVideos).map(([id, data]: [string, unknown]) => ({
      id,
      title: (data as {sceneTitle: string}).sceneTitle || 'Untitled Scene',
  ...
  function moveScene(idx: number, dir: number) {
    ...
    ;[newScenes[idx], newScenes[swap]] = [newScenes[swap], newScenes[idx]]
    setScenes(newScenes)
  }

  function removeScene(id: string) { setScenes(prev => prev.filter(s => s.id !== id)) }

  function updateTransition(id: string, transition: string) {
    setScenes(prev => prev.map(s => s.id === id ? { ...s, transition } : s))
  }

  const totalDuration = scenes.reduce((sum, s) => sum + s.duration, 0)

  const sceneLines = scenes.map((s, i) => String(i+1).padStart(2,'0') + ". " + s.title + " [" + s.duration + "s] → Transition: " + s.transition).join("\n")
  const exportPlan = "EPISODE EDIT PLAN\n" +
    "Generated by Envi Lee Creator Studios Episode Editor\n" +
    "=".repeat(50) + "\n\n" +
    "SCENES IN ORDER:\n" + sceneLines + "\n\n" +
    "TOTAL RUNTIME: " + totalDuration + "s (" + Math.round(totalDuration/60*10)/10 + " min)\n\n" +
    "HOW TO ASSEMBLE IN CAPCUT:\n" +
    "1. Import all video clips in the order above\n" +
    "2. Apply transitions between scenes as noted\n" +
    "3. Add your audio/voiceover track\n" +
    "4. Export at 1080p or 4K\n" +
    "5. Upload to your platforms\n\n" +
    "Generated by Envi Lee Creator Studios"  

  return (
    <div className="pg-in">
      <div style={{ marginBottom: '24px' }}>
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '10px', color: 'rgba(249,249,0,0.5)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '4px' }}>Pipeline Step 3</div>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '32px', fontWeight: 400, color: 'var(--w)', marginBottom: '4px' }}>
          Episode <span style={{ background: 'linear-gradient(135deg,#F9F900,#FF006E)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Editor™</span>
        </div>
        <div style={{ fontSize: '13px', color: 'var(--mu3)' }}>Arrange your scene videos in order, set transitions, and export your episode plan for CapCut or any video editor.</div>
      </div>

      {/* Pipeline */}
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap' as const }}>
        {[['🎬 Episode Builder', false], ['→', false], ['🖼️ Scene Stills', false], ['→', false], ['🎥 Scene Videos', false], ['→', false], ['🎙️ Voice + Lip Sync', false], ['→', false], ['🎞️ Episode Editor', true]].map(([label, active], i) => (
          label === '→' ? <span key={i} style={{ color: 'var(--mu2)', fontSize: '16px' }}>→</span> :
          <span key={i} style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontFamily: "'DM Mono',monospace", background: active ? 'linear-gradient(135deg,#F9F900,#FF006E)' : 'transparent', color: active ? '#000' : 'var(--mu3)', border: active ? 'none' : '0.5px solid rgba(0,207,255,0.15)' }}>{label as string}</span>
        ))}
      </div>
      ...
      {scenes.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '60px 20px', opacity: 0.5 }}>
          <div style={{ fontSize: '48px', marginBottom: '12px', opacity: 0.3 }}>🎞️</div>
          <div style={{ fontSize: '14px', color: 'var(--w)', marginBottom: '8px' }}>No scene videos yet</div>
          <div style={{ fontSize: '12px', color: 'var(--mu3)' }}>Generate scene videos in Scene Video Generator first — they'll appear here automatically</div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px' }}>
          {/* Timeline */}
          <div>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '10px', color: 'rgba(249,249,0,0.4)', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '14px' }}>
              Episode Timeline — {scenes.length} scenes · {totalDuration}s total
            </div>

            {/* Visual timeline bar */}
            <div style={{ display: 'flex', gap: '2px', marginBottom: '20px', height: '40px', borderRadius: '8px', overflow: 'hidden', border: '0.5px solid rgba(249,249,0,0.2)' }}>
              {scenes.map((s, i) => (
                <div key={s.id} onClick={() => setActiveScene(s.id)}
                  style={{ flex: s.duration, background: activeScene === s.id ? 'linear-gradient(135deg,#F9F900,#FF006E)' : `hsl(${i * 40},70%,40%)`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', color: '#fff', fontFamily: "'DM Mono',monospace", minWidth: '30px', transition: 'all .2s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px' }}>
              {scenes.map((s, i) => (
                <div key={s.id} style={{ background: 'var(--s1)', border: `0.5px solid ${activeScene === s.id ? 'rgba(249,249,0,0.3)' : 'rgba(249,249,0,0.1)'}`, borderRadius: '10px', padding: '12px 16px', cursor: 'pointer', transition: 'all .2s' }}
                  onClick={() => setActiveScene(s.id)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '18px', fontWeight: 900, color: 'rgba(249,249,0,0.3)', minWidth: '32px' }}>{String(i+1).padStart(2,'0')}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--w)', marginBottom: '4px' }}>{s.title}</div>
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        <span style={{ fontSize: '10px', color: 'var(--mu3)' }}>{s.duration}s</span>
                        <span style={{ fontSize: '10px', color: 'var(--mu3)' }}>→ {s.transition}</span>
                        <select value={s.transition} onChange={e => { e.stopPropagation(); updateTransition(s.id, e.target.value) }}
                          onClick={e => e.stopPropagation()}
                          style={{ fontSize: '10px', background: 'var(--bg3)', border: '0.5px solid rgba(249,249,0,0.15)', borderRadius: '4px', color: 'var(--mu3)', padding: '2px 6px', cursor: 'pointer' }}>
                          {transitions.map(t => <option key={t}>{t}</option>)}
                        </select>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '4px' }}>
                      {i > 0 && <button onClick={e => { e.stopPropagation(); moveScene(i,-1) }} style={{ padding: '4px 8px', borderRadius: '4px', border: '0.5px solid rgba(249,249,0,0.2)', background: 'transparent', color: 'var(--mu3)', fontSize: '11px', cursor: 'pointer' }}>↑</button>}
                      {i < scenes.length-1 && <button onClick={e => { e.stopPropagation(); moveScene(i,1) }} style={{ padding: '4px 8px', borderRadius: '4px', border: '0.5px solid rgba(249,249,0,0.2)', background: 'transparent', color: 'var(--mu3)', fontSize: '11px', cursor: 'pointer' }}>↓</button>}
                      <button onClick={e => { e.stopPropagation(); removeScene(s.id) }} style={{ padding: '4px 8px', borderRadius: '4px', border: '0.5px solid rgba(255,68,68,0.2)', background: 'transparent', color: '#ff6b6b', fontSize: '11px', cursor: 'pointer' }}>✕</button>
                    </div>
                  </div>
                  {/* Video preview on hover */}
                  {activeScene === s.id && s.videoUrl && (
                    <div style={{ marginTop: '10px' }}>
                      <video src={s.videoUrl} controls style={{ width: '100%', borderRadius: '6px', maxHeight: '200px' }} />
                    </div>
                  )}
                </div>
              ))}
          ...
          {/* Export panel */}
          <div>
            <div className="card hi" style={{ marginBottom: '14px', borderColor: 'rgba(249,249,0,0.2)' }}>
              <div className="ftitle" style={{ color: '#F9F900' }}>Episode Summary</div>
              {[
                { label: 'Total Scenes', val: scenes.length },
                { label: 'Total Runtime', val: `${totalDuration}s` },
                { label: 'Avg Scene Length', val: `${Math.round(totalDuration / scenes.length)}s` },
              ].map(item => (
                <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '0.5px solid rgba(249,249,0,0.07)', fontSize: '12px' }}>
                  <span style={{ color: 'var(--mu3)' }}>{item.label}</span>
                  <span style={{ color: '#F9F900', fontWeight: 700 }}>{item.val}</span>
                </div>
              ))}
            </div>

            <div className="card" style={{ marginBottom: '14px', borderColor: 'rgba(249,249,0,0.15)' }}>
              <div className="ftitle" style={{ color: '#F9F900' }}>Export For</div>
              {[
                { name: 'CapCut', url: 'https://capcut.com', icon: '✂️', desc: 'Import clips in order, apply transitions' },
                { name: 'DaVinci Resolve', url: 'https://blackmagicdesign.com/products/davinciresolve', icon: '🎬', desc: 'Professional color + edit' },
                { name: 'iMovie', url: 'https://apple.com/imovie', icon: '🍎', desc: 'Simple quick assembly on Mac' },
                { name: 'Premiere Pro', url: 'https://adobe.com/premiere', icon: '🎞️', desc: 'Full professional edit suite' },
              ].map(app => (
                <a key={app.name} href={app.url} target="_blank" rel="noreferrer"
                  style={{ display: 'flex', gap: '10px', padding: '10px 12px', background: 'var(--bg3)', borderRadius: '8px', marginBottom: '6px', textDecoration: 'none', alignItems: 'center', border: '0.5px solid rgba(249,249,0,0.08)', transition: 'all .2s' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(249,249,0,0.3)')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(249,249,0,0.08)')}>
                  <span style={{ fontSize: '18px' }}>{app.icon}</span>
                  <div>
                    <div style={{ fontSize: '12px', fontWeight: 600, color: '#F9F900' }}>{app.name}</div>
                    <div style={{ fontSize: '10px', color: 'var(--mu3)' }}>{app.desc}</div>
                  </div>
                </a>
              ))}
            </div>

            <button className="ai-btn" onClick={() => navigator.clipboard?.writeText(exportPlan)} style={{ width: '100%', fontSize: '12px' }}>
              📋 Copy Episode Edit Plan
            </button>
            <div style={{ fontSize: '10px', color: 'var(--mu3)', textAlign: 'center', marginTop: '8px', lineHeight: '1.6' }}>
              Copy the edit plan → paste into notes → import clips into CapCut in order
            </div>
          </div>
        </div>
      )}
