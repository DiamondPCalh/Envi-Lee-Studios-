  ...
  async function generate() {
    ...
    try {
      ...
      if (imgMode === 'elements' && imgElements.length > 0) {
        enhancedPrompt += '. Scene elements: ' + imgElements.map(e => e.label).join(', ') + '. Compose all elements naturally.'
      }
      if (imgMode === 'multichar' && imgChars.length > 0) {
        enhancedPrompt += '. Multiple characters in scene: ' + imgChars.map(c => c.name || 'Character').join(', ') + '. All characters visible and distinct.'
      }

      // Use first char as face reference in multichar mode
      const activeFacePhoto = imgMode === 'multichar' && imgChars.length > 0 ? imgChars[0].url : facePhoto
      const castPhotos = imgMode === 'multichar' ? imgChars.map(c => c.url) : undefined
      ...
      if (activeFacePhoto) setFaceLocked(true)
