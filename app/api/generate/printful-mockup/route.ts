...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'generate_mockup') {
      ...
      const productIdNum = parseInt((body.productId as string) || '0')
      
      const placementStr = (placement as string) || 'default'
      const mockupBody = {
      ...
      // Printful mockup API requires PRODUCT ID in URL, not variant ID
      if (!productIdNum) {
        return NextResponse.json({ 
          error: 'productId required for mockup generation' 
        }, { status: 400 })
      }

      const { ok, status, data } = await pf<{ result?: { task_key?: string; status?: string }; error?: string; code?: number }>(
        apiKey, `/mockup-generator/create-task/${productIdNum}`, 'POST', mockupBody
      )
