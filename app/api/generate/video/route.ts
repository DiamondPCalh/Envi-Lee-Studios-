...
export async function POST(req: NextRequest) {
  try {
    ...
    if (model === 'kling-video' || !model) {
      ...
      if (!keyId || !keySecret) return NextResponse.json({ error: 'HIGGSFIELD_KEY_ID or HIGGSFIELD_API_KEY not configured in Vercel' }, { status: 500 })
      const hfRes = await fetch('https://platform.higgsfield.ai/higgsfield-ai/dop/standard', {
        method: 'POST',
        headers: { 'Authorization': `Key ${keyId}:${keySecret}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ image_url: imageUrl, prompt: `${prompt}. Natural smooth motion, cinematic quality.` }),
      })
      const hfData = await hfRes.json()
      if (!hfRes.ok) throw new Error(hfData.message || 'Higgsfield generation failed')
      return NextResponse.json({ success: true, jobId: hfData.request_id, provider: 'higgsfield' })
    } else {
      result = await generateKlingVideo(imageUrl, prompt, duration || 5)
    }
