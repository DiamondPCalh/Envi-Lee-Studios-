...
function AdminPanel() {
  ...
  const [academy, setAcademy] = useState('baddie')
  const [loading, setLoading] = useState(false)
  const [message, setMessage] = useState('')
  const [students, setStudents] = useState<Record<string, string[]>>({})
  const [activeAcademy, setActiveAcademy] = useState('baddie')

  const academies = [
    { id: 'baddie', label: 'Baddie Academy', color: '#7b3fa0' },
    { id: 'kings', label: 'Kings Academy', color: '#c4973a' },
    { id: 'boss', label: 'Boss Academy', color: '#3a86ff' },
    { id: 'digitalarts', label: 'Digital Arts Academy', color: '#ff006e' },
    { id: 'sparkle', label: 'World of Sparkle Academy', color: '#9B59B6' },
    { id: 'manual', label: 'Manual / Skool', color: '#06B6D4' },
  ]

  useEffect(() => {
    if (isAdmin && user) loadStudents()
  }, [isAdmin, user])

  async function loadStudents() {
    try {
      const res = await fetch(`/api/admin/students?userId=${user?.id}`)
      const data = await res.json()
      if (data.academies) setStudents(data.academies)
  ...
  async function add() {
    ...
    try {
      const res = await fetch('/api/admin/students', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id, email: email.trim(), academy, action: 'add' }),
      })
      const data = await res.json()
      if (data.success) {
        setMessage(`✓ Access granted to ${email.trim()} via ${academies.find(a => a.id === academy)?.label}. Welcome email sent!`)
        setEmail('')
        await loadStudents()
      } else {
        setMessage(`✕ Error: ${data.error}`)
      }
    } catch (e) { setMessage('✕ Failed to add student') }
    setLoading(false)
  }

  async function remove(emailToRemove: string, academyId: string) {
    if (!user) return
    try {
      const res = await fetch('/api/admin/students', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id, email: emailToRemove, academy: academyId, action: 'remove' }),
      })
      const data = await res.json()
  ...
  const currentStudents = students[activeAcademy] || []
  ...
  return (
    <div className="pg-in">
      ...
      <div style={{ fontSize: '13px', color: 'var(--mu3)', marginBottom: '24px' }}>Manage access across all 5 academies — grants instant Redis access + sends welcome email automatically.</div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '10px', marginBottom: '24px' }}>
        {academies.map(a => (
          <div key={a.id} className="card" style={{ textAlign: 'center', padding: '16px', borderColor: `${a.color}30` }}>
            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '24px', fontWeight: 900, color: a.color }}>{(students[a.id] || []).length}</div>
            <div style={{ fontSize: '10px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginTop: '4px' }}>{a.label}</div>
          </div>
        ))}
        <div className="card" style={{ textAlign: 'center', padding: '16px', borderColor: 'rgba(0,255,136,0.2)' }}>
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '24px', fontWeight: 900, color: '#00ff88' }}>{totalStudents}</div>
          <div style={{ fontSize: '10px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginTop: '4px' }}>Total Students</div>
        </div>
      </div>

      {/* Add student */}
      <div className="card hi" style={{ marginBottom: '20px' }}>
        <div className="ftitle">Grant Access to New Student</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '10px', alignItems: 'flex-end' }}>
          ...
          <F label="Academy / Source">
            <select className="fsel" value={academy} onChange={e => setAcademy(e.target.value)}>
              {academies.map(a => <option key={a.id} value={a.id}>{a.label}</option>)}
            </select>
          </F>
          <button className="g-btn" onClick={add} disabled={loading || !email.trim()} style={{ marginBottom: '12px' }}>
            {loading ? 'Adding…' : '+ Grant Access'}
          </button>
        </div>
        {message && (
          <div style={{ marginTop: '10px', padding: '10px 14px', borderRadius: '8px', background: message.startsWith('✓') ? 'rgba(0,255,136,0.08)' : 'rgba(255,68,68,0.08)', border: `0.5px solid ${message.startsWith('✓') ? 'rgba(0,255,136,0.2)' : 'rgba(255,68,68,0.2)'}`, fontSize: '12px', color: message.startsWith('✓') ? '#00ff88' : '#ff6b6b' }}>
            {message}
          </div>
        )}
        <div style={{ marginTop: '12px', padding: '10px 14px', background: 'rgba(6,182,212,0.05)', borderRadius: '8px', border: '0.5px solid rgba(6,182,212,0.15)', fontSize: '11px', color: 'rgba(6,182,212,0.7)', lineHeight: '1.7' }}>
          ✦ Access is granted instantly via Redis — no Vercel env update needed.<br/>
          ✦ A welcome email is automatically sent to the student.<br/>
          ✦ Use <strong>Manual / Skool</strong> for anyone you verify through Skool.
        </div>
      </div>

      {/* Student list by academy */}
      <div>
        <div style={{ display: 'flex', gap: '6px', marginBottom: '16px', flexWrap: 'wrap' as const }}>
          {academies.map(a => (
            <button key={a.id} onClick={() => setActiveAcademy(a.id)}
              style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '11px', cursor: 'pointer', border: `0.5px solid ${activeAcademy === a.id ? a.color : 'rgba(255,255,255,0.1)'}`, background: activeAcademy === a.id ? `${a.color}15` : 'transparent', color: activeAcademy === a.id ? a.color : 'var(--mu3)', fontFamily: "'DM Sans',sans-serif" }}>
              {a.label} ({(students[a.id] || []).length})
            </button>
          ))}
        </div>
        ...
        {currentStudents.length === 0 ? (
          <div className="card" style={{ textAlign: 'center', padding: '32px', opacity: 0.5 }}>
            ...
            <div style={{ fontSize: '13px', color: 'var(--mu3)' }}>No students in {academies.find(a => a.id === activeAcademy)?.label} yet</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px' }}>
            {currentStudents.map(s => (
              <div key={s} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px' }}>
                <div>
                  <div style={{ fontSize: '13px', color: 'var(--w)', fontWeight: 500 }}>{s}</div>
                  <div style={{ fontSize: '10px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace", marginTop: '2px' }}>{academies.find(a => a.id === activeAcademy)?.label}</div>
                </div>
                <button onClick={() => remove(s, activeAcademy)} style={{ padding: '5px 12px', borderRadius: '6px', border: '0.5px solid rgba(255,68,68,0.3)', background: 'rgba(255,68,68,0.06)', color: '#ff6b6b', fontSize: '11px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
                  Revoke Access
                </button>
              </div>
            ))}
...
export default function CreatorSuitePage() {
  ...
  return (
    <>
      ...
      <SignedIn>
        <UniversalAccessGate appName="Creator Suite™">
