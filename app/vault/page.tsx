...
interface TwinVersion {
  id: string; type: string; description: string; active: boolean
}
  ...
  useEffect(() => {
    async function check() {
      ...
      try {
        ...
        const res = await fetch(`/api/access/vault?userId=${user.id}&email=${encodeURIComponent(email)}`)
        const data = await res.json()
...
function StudentVideoEditor({ pipeline, onSubmit }: { pipeline?: VideoPost | null; onSubmit: (post: VideoPost) => void }) {
  ...
  async function handleSubmit() {
    ...
    // Check if student has submitted less than 2 videos — lock them out if so
    // They need at least 2 submissions to maintain access
    // If this is their first submission, send a reminder
    // If they somehow lost access, the lockout email would have already been sent
    // This just tracks their progress toward the 2-video requirement
...
// ── VAULT DNA SELECTOR ────────────────────────────────────────
function VaultDNASelector({ userId, onSelect, selected }: {
  userId: string; onSelect: (dna: VaultDNA | null) => void; selected: VaultDNA | null
}) {
  const [dnas, setDnas] = useState<VaultDNA[]>([])
  const [versions, setVersions] = useState<TwinVersion[]>([])
  const [open, setOpen] = useState(false)

  useEffect(() => {
    const savedDNA = JSON.parse(localStorage.getItem(`dna_${userId}`) || '[]')
    const savedVersions = JSON.parse(localStorage.getItem(`versions_${userId}`) || '[]')
    setDnas(savedDNA)
  ...
  if (dnas.length === 0) return (
    <div style={{ background: 'var(--yg)', border: '0.5px solid var(--yb)', borderRadius: '8px', padding: '10px 14px', fontSize: '12px', color: 'var(--yellow)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <span>✦ No Baddie DNA™ profiles yet — create one in the Prompt Bank</span>
      <a href="/prompts" style={{ fontSize: '11px', color: 'var(--cyan)', textDecoration: 'none', border: '0.5px solid var(--cb)', padding: '3px 10px', borderRadius: '5px' }}>Go to Prompt Bank →</a>
    </div>
  )
    ...
    <div style={{ marginBottom: '16px' }}>
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' as const }}>
        ...
        <div style={{ position: 'relative' as const }}>
          <button onClick={() => setOpen(!open)}
            style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 14px', borderRadius: '8px', border: `0.5px solid ${selected ? 'var(--yb)' : 'rgba(0,207,255,0.2)'}`, background: selected ? 'var(--yg)' : 'var(--s1)', color: selected ? 'var(--yellow)' : 'var(--mu3)', fontSize: '12px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
            <span>◉</span>
            <span>{selected ? `DNA: ${selected.name}` : 'Select Baddie DNA™'}</span>
            <span style={{ fontSize: '10px' }}>▾</span>
          </button>
          {open && (
            <div style={{ position: 'absolute' as const, top: '110%', left: 0, background: 'var(--bg2)', border: '0.5px solid rgba(0,207,255,0.2)', borderRadius: '10px', padding: '8px', zIndex: 100, minWidth: '220px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
              <div onClick={() => { onSelect(null); setOpen(false) }}
                style={{ padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: 'var(--mu3)', marginBottom: '4px' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'var(--cg)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
                No DNA — use default
              </div>
              {dnas.map(dna => (
                <div key={dna.id} onClick={() => { onSelect(dna); setOpen(false) }}
                  style={{ padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', background: selected?.id === dna.id ? 'var(--yg)' : 'none' }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'var(--yg)')}
                  onMouseLeave={e => (e.currentTarget.style.background = selected?.id === dna.id ? 'var(--yg)' : 'none')}>
                  <div style={{ fontSize: '12px', color: 'var(--yellow)', fontWeight: 600 }}>{dna.name}</div>
                  <div style={{ fontSize: '10px', color: 'var(--mu3)', fontFamily: "'DM Mono',monospace" }}>{dna.skinTone} · {dna.hairTexture}</div>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Active twin version badge */}
        {activeVersion && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', borderRadius: '8px', border: '0.5px solid var(--cb)', background: 'var(--cg)', fontSize: '12px', color: 'var(--cyan)' }}>
            <span>✦</span>
            <span>{activeVersion.type}</span>
            <a href="/prompts" style={{ fontSize: '10px', color: 'var(--mu3)', textDecoration: 'none', marginLeft: '4px' }}>Switch →</a>
          </div>
        )}

        {selected && (
          <div style={{ fontSize: '11px', color: 'var(--yellow)', fontFamily: "'DM Mono',monospace" }}>
            ✦ DNA active — recreating as {selected.name}
          </div>
        )}
      </div>
      ...
      {selected && (
        <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' as const, marginTop: '8px' }}>
          {[selected.skinTone, selected.hairTexture, selected.hairColor, selected.bodyType, selected.vibe].filter(Boolean).map(tag => (
            <span key={tag} style={{ fontSize: '9px', padding: '2px 8px', background: 'var(--yg)', border: '0.5px solid var(--yb)', borderRadius: '20px', color: 'var(--yellow)', fontFamily: "'DM Mono',monospace" }}>{tag}</span>
          ))}
        </div>
      )}
    </div>
...
function StudentSuite({ pipeline, onBack }: { pipeline?: VideoPost | null; onBack: () => void }) {
  ...
  const [selectedDNA, setSelectedDNA] = useState<VaultDNA | null>(null)
  ...
  return (
    <div className="pg-in">
      ...
      {/* Baddie DNA™ + AI Twin Versions selector */}
      {user && (
        <VaultDNASelector
          userId={user.id}
          selected={selectedDNA}
          onSelect={setSelectedDNA}
        />
      )}
      ...
      {activeTab === 'pipeline' && pipeline && (
        <div>
          <div style={{ background: 'var(--yg)', border: '0.5px solid var(--yb)', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', fontSize: '12px', color: 'var(--yellow)' }}>
            ✦ Full production pipeline from: <strong>{pipeline.title}</strong> — use this to recreate with {selectedDNA ? <strong style={{ color: 'var(--cyan)' }}>{selectedDNA.name}</strong> : 'your own AI twin'}
          </div>
          {selectedDNA && (
            <div style={{ background: 'var(--cg)', border: '0.5px solid var(--cb)', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontSize: '11px', color: 'var(--cyan)', fontFamily: "'DM Mono',monospace" }}>
              ◉ Baddie DNA™ active — all prompts will be adapted for {selectedDNA.name} · {selectedDNA.skinTone} · {selectedDNA.bodyType}
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {[
              { title: 'Scripts & Dialogue', content: pipeline.workflow?.slice(0, 200) + '…' },
              { title: 'Scene Prompts', content: pipeline.scenes.map((s, i) => `Scene ${i+1}: ${selectedDNA ? `[Adapted for ${selectedDNA.name}, ${selectedDNA.skinTone} skin, ${selectedDNA.hairTexture} ${selectedDNA.hairColor} hair] ` : ''}${s.stillPrompt}`).join('\n') },
...
function AdminRoom() {
  ...
  async function approve(post: VideoPost) {
    ...
    // Unlock user when their video is approved
    try {
      await fetch('/api/vault/lockout', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: post.creatorId }),
      })
    } catch (e) { console.error('[unlock error]', e) }
  }

  async function lockoutUser(userId: string, userEmail: string, userName: string) {
    try {
      await fetch('/api/vault/lockout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert(`Lockout email sent to ${userEmail}`)
          ...
          {pending.length === 0 ? (
            ...
            <div key={post.id} className="card hi" style={{ marginBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={() => lockoutUser(post.creatorId, '', post.creatorName)} style={{ padding: '7px 14px', borderRadius: '7px', border: '0.5px solid rgba(255,230,0,0.3)', background: 'transparent', color: 'var(--yellow)', fontSize: '11px', cursor: 'pointer' }}>🔒 Lock</button>
              </div>
            </div>
          ))}
