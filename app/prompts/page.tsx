...
type ActiveTab = 'prompts' | 'generate' | 'calendar' | 'stacks' | 'dna' | 'twins' | 'aigenerator' | 'reverse' | 'categories'
  ...
  useEffect(() => {
    async function check() {
      ...
      try {
        ...
        const res = await fetch(`/api/access/prompts?userId=${user.id}&email=${encodeURIComponent(email)}`)
        const data = await res.json()
        setHasAccess(data.hasAccess || data.role === 'test' || data.role === 'admin')
...
function AdminRoom({ isAdmin }: { isAdmin: boolean }) {
  ...
  function addCategory() {
    if (!newCat.trim() || allCategories.includes(newCat.trim())) return
  ...
  return (
    <div>
      ...
      {adminTab === 'categories' && (
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '18px', fontWeight: 800, color: 'var(--w)', marginBottom: '4px' }}>Category Manager</div>
          <div style={{ fontSize: '12px', color: 'var(--mu3)', marginBottom: '20px' }}>Add your own custom categories to the Prompt Bank. They show up everywhere — main library, student suite, and one click generator.</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            <div className="card hi">
              <div className="ftitle">Add New Category</div>
              <F label="Category name">
                <input className="finp" placeholder="e.g. Baddie Travel, Luxury Mom, AI Model" value={newCat} onChange={e => setNewCat(e.target.value)} onKeyDown={e => e.key === 'Enter' && addCategory()} />
              </F>
              <button className="p-btn" onClick={addCategory} disabled={!newCat.trim()} style={{ width: '100%', fontSize: '12px' }}>+ Add Category</button>
              {customCategories.length > 0 && (
                <div style={{ marginTop: '16px' }}>
                  <div className="ftitle">Your Custom Categories</div>
                  <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '6px' }}>
                    {customCategories.map(cat => (
                      <div key={cat} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--bg3)', border: '0.5px solid rgba(255,111,216,0.2)', borderRadius: '8px' }}>
                        <span style={{ fontSize: '12px', color: 'var(--pink)' }}>{cat}</span>
                        <button onClick={() => removeCategory(cat)} className="del-btn" style={{ fontSize: '10px', padding: '3px 8px' }}>Remove</button>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="card">
              <div className="ftitle">All Categories ({allCategories.length})</div>
              <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '6px', maxHeight: '400px', overflowY: 'auto' }}>
                {allCategories.map(cat => (
                  <span key={cat} style={{ fontSize: '10px', padding: '3px 10px', background: customCategories.includes(cat) ? 'var(--pg)' : 'var(--s2)', border: `0.5px solid ${customCategories.includes(cat) ? 'var(--pb)' : 'rgba(192,132,252,0.1)'}`, borderRadius: '20px', color: customCategories.includes(cat) ? 'var(--pink)' : 'var(--mu3)', fontFamily: "'DM Mono',monospace" }}>
...
// ── SUITE PROMPT CARD ─────────────────────────────────────────
function SuitePromptCard({ p, theme, onDelete }: {
  p: { id: number; prompt: string; category: string; shared: boolean }
  theme: SuiteTheme
  ...
  return (
    <div style={{ background: 'var(--s1)', border: `0.5px solid ${theme.primaryColor}25`, borderRadius: '12px', overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px' }}>
        <div style={{ padding: '14px', borderRight: `0.5px solid ${theme.primaryColor}15` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
              ...
              <button onClick={() => onDelete(p.id)} className="del-btn" style={{ fontSize: '10px', padding: '2px 7px' }}>✕</button>
            </div>
          </div>
          <div style={{ fontSize: '12px', color: 'var(--w2)', lineHeight: '1.6', marginBottom: '10px' }}>
            {expanded ? p.prompt : p.prompt.slice(0, 120) + (p.prompt.length > 120 ? '…' : '')}
          </div>
          <div style={{ display: 'flex', gap: '6px' }}>
            <button onClick={() => setExpanded(!expanded)}
              style={{ padding: '4px 10px', borderRadius: '6px', fontSize: '10px', cursor: 'pointer', border: `0.5px solid ${theme.primaryColor}40`, background: 'transparent', color: theme.primaryColor, fontFamily: "'DM Sans',sans-serif" }}>
              {expanded ? 'Hide ↑' : 'View Full →'}
            </button>
            <button onClick={copy}
              style={{ padding: '4px 10px', borderRadius: '6px', fontSize: '10px', cursor: 'pointer', border: `0.5px solid ${theme.accentColor}40`, background: 'transparent', color: theme.accentColor, fontFamily: "'DM Sans',sans-serif" }}>
              {copied ? '✓ Copied!' : 'Copy'}
            </button>
          </div>
          {expanded && (
            <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: `0.5px solid ${theme.primaryColor}15` }}>
              <div style={{ fontSize: '10px', color: theme.primaryColor, fontFamily: "'DM Mono',monospace", textTransform: 'uppercase' as const, letterSpacing: '.7px', marginBottom: '8px' }}>Take this prompt to:</div>
              <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' as const }}>
                {VIDEO_APPS.map(app => (
                  <a key={app.format} href={app.url} target="_blank" rel="noreferrer"
                    style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 8px', background: 'var(--s2)', border: '0.5px solid rgba(192,132,252,0.15)', borderRadius: '5px', textDecoration: 'none', fontSize: '10px', color: 'var(--w2)' }}>
                    <span>{app.icon}</span>{app.name}
                  </a>
                ))}
              </div>
        ...
        <div style={{ background: 'var(--bg3)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', minHeight: '160px', position: 'relative' as const, cursor: imageUrl ? 'default' : 'pointer' }}
          onClick={() => !imageUrl && !generating && generateImage()}>
          {generating ? (
            <div style={{ textAlign: 'center', padding: '16px' }}>
              <div className="lbar" style={{ width: '50px', margin: '0 auto 8px' }}><div className="lbar-fill" /></div>
              <div style={{ fontSize: '10px', color: theme.primaryColor }}>Generating…</div>
            </div>
          ) : imageUrl ? (
            <>
              <img src={imageUrl} alt="Generated" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              <div style={{ position: 'absolute', bottom: '5px', right: '5px', display: 'flex', gap: '3px' }}>
                <a href={imageUrl} download style={{ padding: '3px 7px', borderRadius: '4px', background: 'rgba(0,0,0,0.8)', color: theme.primaryColor, fontSize: '10px', textDecoration: 'none' }}>⬇</a>
                <button onClick={() => { setImageUrl(''); generateImage() }} style={{ padding: '3px 7px', borderRadius: '4px', background: 'rgba(0,0,0,0.8)', color: theme.accentColor, fontSize: '10px', border: 'none', cursor: 'pointer' }}>↺</button>
              </div>
            </>
          ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
            {myPrompts.map(p => (
              <SuitePromptCard
                ...
                theme={theme}
                onDelete={(id) => {
                  const updated = myPrompts.filter(mp => mp.id !== id)
                  setMyPrompts(updated)
                  localStorage.setItem(`myPrompts_${user?.id}`, JSON.stringify(updated))
                }}
              />
            ...
            {myPrompts.length === 0 && (
              <div className="card" style={{ textAlign: 'center', padding: '40px', opacity: 0.5 }}>
                ...
                <div style={{ fontSize: '11px', color: 'var(--mu2)', marginTop: '4px' }}>Add a prompt above — photo auto-generates beside it</div>
              </div>
            )}
          </div>
        </div>
